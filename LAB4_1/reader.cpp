...
int main () {
    ...
    if (handleM){
        for (int i = 0; i < 3; i++){
            int page = WaitForMultipleObjects(pgCount, usedSemaphores.data(), false, INFINITE);
            logger = "taking used semaphores for page " + to_string(page) + " at " + to_string(GetTickCount()) + "\n";
            ...
            logger = "starting reade at " + to_string(GetTickCount()) + "\n";
            WriteFile (out, logger.data(), logger.length(), nullptr, nullptr);
            SleepEx(rand() % 1000 + 500, false);
            logger = "stop read at " + to_string(GetTickCount()) + "\n";
            WriteFile (out, logger.data(), logger.length(), nullptr, nullptr);
            ...
            if (ReleaseSemaphore(unusedSemaphores[page], 1, nullptr))
            {
